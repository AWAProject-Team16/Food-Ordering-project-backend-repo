...
router.get("/type/:restaurant_type", function (req, res) {
  restaurants.foodCategoryRestaurants(
    req.params.restaurant_type,
    function (err, dbResult) {
      if (err) {
        res.status(500).json(err);
      } else {
...
router.get(
  ...
  (req, res) => {
    restaurants.getManagerRestaurants(
      req.user.idusers,
      function (err, dbResult) {
        if (err) {
          res.status(500).json(err);
...
router.delete(
  "/:restaurant_id/deleteRestaurant",
  passport.authenticate("jwt", { session: false }),
  function (req, res) {
