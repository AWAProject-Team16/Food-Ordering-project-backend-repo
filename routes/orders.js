    ...
    orders.addOrder(req.user.idusers, req.body, function (err, dbResult) {
      if (err) {
        ...
        if (dbResult.affectedRows == 0) {
          res.status(400).json({
            Status:
              400 + ", Cannot add order. For help, contact the IT manager",
          });
        } else {
          orders.addOrderDetails(
            dbResult.insertId,
...
router.get(
  "/confirmDelivered/:idorders",
  passport.authenticate("jwt", { session: false }),
  (req, res) => {
    orders.customerConfirmDelivered(
      req.user.idusers,
      req.params.idorders,
      (err, dbResult) => {
        if (err) {
