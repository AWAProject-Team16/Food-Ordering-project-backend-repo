...
router.get("/totalRevenueOrdersCustomersForManager", passport.authenticate("jwt", { session: false }), (req, res) => {
  users.getTotalRevenueOrdersCustomersForManager(req.user.idusers, (err, dbResult) => {
    if (err) {
      console.error(err);
      res.sendStatus(500);
...
router.get("/most4PopularDaysForManager", passport.authenticate("jwt", { session: false }), (req, res) => {
...
router.get("/most6PopularTimeForManager", passport.authenticate("jwt", { session: false }), (req, res) => {
...
router.get("/most3PopularProductsForManager", passport.authenticate("jwt", { session: false }), (req, res) => {
  users.getMost3PopularProductsForManager(req.user.idusers, (err, dbResult) => {
    if (err) {
      console.error(err);
      res.sendStatus(500);
    } else res.json(dbResult);
...
router.get("/totalRestaurants", passport.authenticate("jwt", { session: false }), (req, res) => {
